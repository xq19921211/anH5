<template>
  <div class="wrapper">
    <van-popup :close-on-click-overlay="false" position="bottom" class="gorate" v-model="workedInfo.isShow">
      <div class="title clearfix">
        <div class="title__name">确认{{labelDesc}}</div>
        <div class="title__close" @click="_close">
          <van-icon name="close" />
        </div>
      </div>
      <div class="body">
        <!-- <div class="label" v-if="workedInfo.row">确认{{labelDesc}}【{{workedInfo.row.name}}】吗？</div> -->
        <div class="imgwrap">
          <div class="img">
            <div class="img__item">
        ...
        <div class="time">
          ...
          <span>{{workedInfo.times | dateFormat}}</span>
        </div>
      </div>
      <div class="footer">
        <van-button type="danger" style="border-radius:0;" :block="true" @click="_addOrderOperateTime">
          {{labelDesc}}
        </van-button>
      </div>
    </van-popup>
...
import {addOrderOperateTime} from '@/api';
export default {
  props: {
    workedInfo: {
      type: Object,
  ...
  watch: {
    'workedInfo.times'() {
      if (this.workedInfo.row.operateOrderType == 1) {
        this.labelDesc = '上班打卡';
      } else if (this.workedInfo.row.operateOrderType == 2) {
        this.labelDesc = '下班打卡';
      } else if (this.workedInfo.row.operateOrderType == 3) {
        this.labelDesc = '打卡';
      }
    },
  },
  methods: {
    ...
    /**
     * 确认作业
     * @param {string} operatetype 操作id
     */
    _addOrderOperateTime() {
      if (this.fileid1) {
      ...
      let req = {
        //操作id
        operatetype: this.workedInfo.operatetype,
        //订单id
        orderid: this.workedInfo.row.orderid,
        //用户id
